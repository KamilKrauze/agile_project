...
<body>
    ...
    <div class="container-fluid mx-2 my-2">
        ...
        <?php
        try {
            function remItem($rid){
                echo 'hello';
                $dfr = $pdo->prepare("DELETE FROM Recipes WHERE RecipeID=?");
                $dfr->bindParam(1, $rid, PDO::PARAM_INT);
                $dfr->execute();

                $dfri = $pdo->prepare("DELETE FROM recipeingredients WHERE RecipeID=?");
                $dfri->bindParam(1, $rid, PDO::PARAM_INT);
                $dfri->execute();
            }
            $fetchRecipes = "SELECT RecipeID, RecipeName FROM recipes";
            ...
            while ($row = $stmt->fetch()) {
                ...
                <div class="modal fade" id="recipeModal-'.$id.'" tabindex="-1" aria-labelledby="exampleRecipeLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#rusureModal-'.$id.'">Remove</button>
                ...
                echo '
                <div class="modal fade" id="rusureModal-'.$id.'" tabindex="-1" aria-labelledby="exampleRecipeLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                        <div class="modal-body">
                        ...
                        <div class="modal-footer">
                            ...
                            <button name="id" type="submit" class="btn btn-green" value="'.$id.'">Yes</button>
                            </form>
                        </div>
                        </div>
                    </div>
